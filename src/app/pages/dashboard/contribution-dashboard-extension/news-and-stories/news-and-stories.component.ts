...
export class NewsAndStoriesComponent implements OnInit {
  ...
  fetchNews(from: number = 0) {
    this.loading = true;
    this.stakeholderNewsService.getStakeholderNews(this.stakeholderId, from, this.pageSize).subscribe({
  ...
  goToPage(page: number) {
    this.fetchNews((page - 1) * this.pageSize);
  }
  ...
  onDelete() {
    ...
    this.stakeholderNewsService.deleteNews(this.selectedDeleteStakeholderId, this.selectedDeleteId)
      ...
      .subscribe({
        next: () => {
          this.deleting = false;
          this.newsItems = this.newsItems.filter(item => item.result.id !== this.selectedDeleteId);
          this.total--;

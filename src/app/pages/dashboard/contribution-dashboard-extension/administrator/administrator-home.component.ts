...
export class AdministratorHomeComponent implements OnInit {
  ...
  private administrator$ = this.route.params.pipe(
    map(params => params['id'] as string),
    filter(id => !!id),
    switchMap((id: string): Observable<Administrator> => {
      const storedJson = sessionStorage.getItem("currentAdministrator");
      const storedGroup = storedJson ? (JSON.parse(storedJson) as Administrator) : null;

      if (storedGroup && storedGroup.id === id) {
        return of(storedGroup);
      }
      return this.stakeholdersService.getAdministratorById(id);
    }),
    tap(admin => this.userService.changeCurrentAdministrator(admin)),
    shareReplay(1)
  );

  private surveyRawData$ = this.administrator$.pipe(
